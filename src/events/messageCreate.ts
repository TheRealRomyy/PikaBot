...
class MessageEvent extends Event {
    ...
    async run(message : Message) {
        ...
        if(message.channel.type === "DM") {
            ...
            let pikaplouf = this.client.guilds.cache.get("843933749434646568");

            if(pikaplouf.members.cache.get(message.author.id) == null) return;
            if(!message.content) return;
            
            let hasAlreadyATicket = Boolean(pikaplouf.channels.cache.find(channel => channel.name === `ticket-${message.author.id}`));

            let ticket : any = hasAlreadyATicket ? pikaplouf.channels.cache.find(channel => channel.name === `ticket-${message.author.id}`) : await pikaplouf.channels.create(`ticket-${message.author.id}`);
            ...
            let group = new MessageActionRow().addComponents([ closeButton ]);
            ...
            if(hasAlreadyATicket) {
                ...
                let msg = await (ticket as TextChannel).send({
                    embeds: [embed],
        ...
        if(message.channel.name.includes("ticket-")) {
            ...
            let userID = message.channel.name.split("-")[1];
